...
public class QuoteEmail {
        ...
        for (QuoteDetail det : detalles) {
            String producto = "Servicio";
             if (det.getProduct() != null) {
                // Carga el producto explícitamente usando el repositorio
                Product prod = productRepository.findById(det.getProduct().getId()).orElse(null);
                producto = prod != null ? prod.getType() : "Producto";
            } else if (det.getService() != null) {
                Service serv = serviceRepository.findById(det.getService().getId()).orElse(null);
                producto = serv != null ? serv.getType() : "Servicio";
            }
    ...
    public void sendEmail(String emailDestino, String nombreCliente, String quoteId, String htmlMsg, String quoteCod){
           try {
            ...
            try {
                logger.info("Intentando enviar correo de cotizacion a {}", emailDestino);

                email.send();

                logger.info("Correo de cotizacion enviado correctamente a {}", emailDestino);
