...
npm install
```
(Opcional) Configura variables de entorno, como las credenciales de EmailJS, en un archivo `.env` o en el Client/src/pages/Contact.tsx
...
Crea la base de datos en SQL Server con el nombre 'NegocioWebIntegrado'
``` SQL Server
create database NegocioWebIntegrado
```

Recuerda tener habilitado y configurado el puerto 1433 por el cual se conectara SQL Server para la conección a la base de datos y con las credenciales validas para que se valide la conección. 
...
Si ocurre algún error al eliminar el archivo 'target' en WebPage puedes eliminarlo tambien manualmente
``` Ruta
\Server\WebPage
``` 
...
Si deseas ver los productos ya registrados y el catalogo completo, puedes ejecutar los inserts que estan en:
```Ruta
db/inserts.sql
```
...
Usa las credenciales de prueba () para iniciar sesión o regístrate como nuevo usuario.  
...
Esta sección ofrece una visión general de las funcionalidades de la aplicación para usuarios finales. Para detalles completos, consulta el archivo [SANNA_manual_usuario.pdf](Docs/SANNA_manual_usuario.pdf).
